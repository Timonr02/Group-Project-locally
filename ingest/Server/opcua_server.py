...
async def main():
    ...
    last_job_duration_s = await status.add_variable(idx, "last_job_duration_s", 0)
    count_finished_card = await status.add_variable(idx, "count_finished_card", 0)
    ...
    async with server:
        ...
        while True:
            ...
            progress_val = (progress_val + 10) % 100

            await progress.write_value(progress_val)
            await last_job_duration_s.write_value(progress_val * 2)
            await count_finished_card.write_value(progress_val // 20)
